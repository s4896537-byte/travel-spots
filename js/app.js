...
function categoryEmoji(cat) {
  const map = { '自然風景':'🌿','文化歷史':'🏛️','美食':'🍜','購物':'🛍️','主題樂園':'🎡','海灘':'🏖️','山岳':'⛰️','城市探索':'🏙️','其他':'📌' };
  return map[cat] || '📍';
}
...
function starsHTML(rating, cls = 'card-rating') {
  ...
  for (let i = 1; i <= 5; i++) html += `<span class="s${i <= rating ? ' lit' : ''}">★</span>`;
  return html + '</div>';
}
...
async function loadSpots() {
  ...
  try {
    ...
    allSpots = data || [];
    renderAll();
    console.log(`✅ 載入 ${allSpots.length} 筆`);
  } catch (e) {
...
function renderAll() {
  const sc = document.getElementById('spotsContainer');
  const es = document.getElementById('emptyState');
  const list = getFiltered();
  updateStats();
  if (list.length === 0) { es.style.display='flex'; sc.innerHTML=''; return; }
  es.style.display = 'none';
  sc.innerHTML = list.map(s => cardHTML(s)).join('');
  sc.querySelectorAll('.spot-card').forEach(card => {
    const id = parseInt(card.dataset.id);
    card.addEventListener('click', e => { if (!e.target.closest('.card-action-btn')) openDetail(id); });
    card.querySelector('.btn-edit-card')?.addEventListener('click', e => { e.stopPropagation(); openEdit(id); });
    card.querySelector('.btn-delete-card')?.addEventListener('click', e => { e.stopPropagation(); confirmDelete(id); });
  });
}

function cardHTML(s) {
  const img = s.image_url
    ? `<img class="card-image" src="${escapeHtml(s.image_url)}" alt="${escapeHtml(s.name)}" loading="lazy" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'"><div class="card-image-placeholder" style="display:none">${categoryEmoji(s.category)}</div>`
    : `<div class="card-image-placeholder">${categoryEmoji(s.category)}</div>`;
  const tags = s.tags ? s.tags.split(',').filter(t=>t.trim()).slice(0,3).map(t=>`<span class="tag-chip">#${escapeHtml(t.trim())}</span>`).join('') : '';
  return `
<div class="spot-card${s.status==='已去'?' visited-card':''}" data-id="${s.id}">
  ...
  ${img}
  <div class="card-body">
    <div class="card-top"><div class="card-name">${escapeHtml(s.name)}</div></div>
    <div class="card-location"><i class="fas fa-map-marker-alt"></i> ${escapeHtml([s.city,s.country].filter(Boolean).join('・'))}</div>
    <div class="card-badges">
      ...
      ${s.budget?`<span class="badge badge-category">${s.budget}</span>`:''}
      ${s.best_season?`<span class="badge badge-category">${seasonLabel(s.best_season)}</span>`:''}
    </div>
...
function openAddModal() {
  ...
  document.getElementById('spotId').value=document.getElementById('fRating').value='0';
...
function openEdit(id) {
  ...
  editingId=id; currentRating=s.rating||0;
...
async function saveSpot() {
  ...
  try {
    if(editingId){
      const{data,error}=await sbClient.from(TABLE).update(payload).eq('id',editingId).select().single();
      if(error) throw error;
  ...
  } catch(e){console.error(e);showToast('儲存失敗：'+e.message,'error');}
  finally{btn.disabled=false;btn.innerHTML='<i class="fas fa-save"></i> 儲存景點';}
}
...
function openDetail(id){
  ...
  document.getElementById('detailLocation').textContent=[s.city,s.country].filter(Boolean).join('・');
  const iw=document.getElementById('detailImageWrap');
  iw.innerHTML=s.image_url?`<img src="${escapeHtml(s.image_url)}" alt="${escapeHtml(s.name)}" onerror="this.parentElement.innerHTML=''"/>`:'';
  ...
  document.querySelector('.detail-tags-row')?.remove();
  if(s.tags){
    const td=document.createElement('div');
    td.className='detail-tags-row';
    td.style.cssText='display:flex;flex-wrap:wrap;gap:6px;margin-bottom:14px';
    td.innerHTML=s.tags.split(',').filter(t=>t.trim()).map(t=>`<span class="tag-chip">#${escapeHtml(t.trim())}</span>`).join('');
    meta.after(td);
  }
  const ne=document.getElementById('detailNotes');
  ne.style.display=s.notes?'block':'none';
  if(s.notes) ne.textContent=s.notes;
  document.getElementById('detailEditBtn').onclick=()=>{document.getElementById('detailOverlay').classList.remove('open');openEdit(id);};
  document.getElementById('detailDeleteBtn').onclick=()=>{if(confirm(`確定刪除「${s.name}」？`))deleteSpot(id);};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('starRating').addEventListener('mouseover',e=>{
    if(e.target.classList.contains('star')){
      ...
      document.querySelectorAll('#starRating .star').forEach(s=>s.classList.toggle('hover',parseInt(s.dataset.val)<=hv));
    }
  });
  ...
  document.getElementById('viewGrid').addEventListener('click',()=>{
    viewMode='grid';
    document.getElementById('spotsContainer').classList.remove('list-view');
    document.getElementById('viewGrid').classList.add('active');
    document.getElementById('viewList').classList.remove('active');
  });
  document.getElementById('viewList').addEventListener('click',()=>{
    viewMode='list';
    document.getElementById('spotsContainer').classList.add('list-view');
    document.getElementById('viewList').classList.add('active');
    document.getElementById('viewGrid').classList.remove('active');
  });

  let searchTimer;
  document.getElementById('searchInput').addEventListener('input',()=>{clearTimeout(searchTimer);searchTimer=setTimeout(renderAll,300);});
  ...
  document.addEventListener('keydown',e=>{
    if(e.key==='Escape'){closeModal();document.getElementById('detailOverlay').classList.remove('open');}
  });
});
